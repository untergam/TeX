...
to infinite intervals. The first idea was to use the work from \cite{HV_hoq}
on finite intervals and from \cite{H_nsd_sii} on semi-finite intervals
to construct a solution for infinite intervals. In this process it turned out
that the proof of the latter is flawed. The next step is then about the correct
handling of the stationary point since our single stationary point is in general
complex. There are some comments on that situation in the first reference.
...
itself become rapidly oscillatory and we end up with another oscillatory integral,
...
quadrature nodes that we have to use for correct integration. One might try to resolve
these oscillations by another method on top of what we did so far.

The next problem is about integrals $\braket{\phi|\hat{o}|\phi}$ including additional
operators $\hat{o}$. An example of this kind is the computation of expectation
values of potential energies: $\braket{\phi|V|\phi}$. \footnote{Kinetic energy
...
In the example given, the parameter sets of both wavepackets match and therefore
the integration is much easier, oscillations caused by the exponential parts cancel
out to a large degree. But let us consider the general case where we have
$\braket{\phi|\hat{o}|\phi^\prime}$. The main issue here are the possible effects
on the oscillator $g$ induced by $\hat{o}$. For example, assume that $\hat{o}$ is a
potential $V(x)$. Then, $V$ becomes part of the envelope and there apply
some growth limits on the envelope $f$ for $|x| \rightarrow \infty$. If $V$ contains
exponential parts, they have to be merged into the oscillator $g$ and in turn change the
layout of the integration paths. Further, if there are any poles in $f$, we have to
be very careful with integration too and include all the residuals. In the computations
done so far, the envelope $f$ is assumed to have no poles in $\mathbb{C}^{d}$
Altogether this poses some challenges for non-polynomial potentials $V$.
...
like hyperbolic cuts might be beneficent. One might consider adaptive versions too.
