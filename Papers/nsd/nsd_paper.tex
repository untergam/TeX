...
$g(x_1, \ldots, x_n):\mathbb{R}^N\rightarrow\mathbb{C}$, we want to compute the integral:
...
Computing that integral above by direct standard numerical quadrature schemes
...
integrand $I$. Since our interests stem from quantum
mechanics, we anticipate the integrand $I$ being composed of wavepackets $\phi$ and
...
where we can not afford to use such many quadrature nodes.
It turns out that we need much better methods to efficiently deal with this
kind of integrals as \eqref{eq:hoi} and obtain mathematically correct results.
Therefore the theory of \emph{numerical steepest descent} was developed about ten
years ago exactly for these integrals. For more details on the mathematical theory
see the work by Huybrechs, Vandewalle and some other People. The main technique
is described in \cite{HV_hoq} for the one-dimensional case and in \cite{HV_cub}
for multivariate integrands. Semi-infinite intervals are treated in \cite{H_nsd_sii}.
...
the computation of overlap integrals from quantum mechanics.
...
In this section we look at the wavepackets $\phi$ involved in the computation
...
and in the $D$ dimensional case counted by a multi-index vector
...
\begin{equation}
  \phi(\vec{x}) \sim p\left(\vec{x}\right)
...
What looks like a big issue in the first moment is actually a very lucky coincidence.
...
Usually we are interested in overlap integrals like $\dotp{\phi_k}{\phi_l}$.
...
\eqref{eq:oscillatory_integral} we started with. But first we would like to
show a concrete example in figure \ref{fig:overlap_example} of how the wavepackets
involved and the integrand look like.
...
\begin{figure}[h!]
  ...
  \begin{subfigure}[t]{0.5\linewidth}
    ...
    \caption{Integrand consisting of $\phi_1$ and $\phi_2$.}
  ...
  \begin{subfigure}[t]{0.5\linewidth}
    ...
    \caption{Integrand consisting of $\phi_8$ and $\phi_6$.}
    \label{fig:overlap_example_integrand_2}
  \end{subfigure}
  \caption{\label{fig:overlap_example}
  Wavepackets $\phi_k$ and $\phi_l$ and the integrand $\conj{\phi_k} \phi_l$.
  ...
  and the $\varepsilon$ is not very small.}
\end{figure}
...
or conjugate somewhere. To simplify the notation define the matrix:
...
Finally there is the domain of integration denoted by $\Omega$.
In the theory so far this is a bounded subset of $\mathbb{R}^D$.
Figure \ref{fig:oscillatory_example_2d} shows a particular
example of such an integrand.

\marginpar{recheck domains}

\begin{figure}[h]
  \centering
  \includegraphics[width=0.6\linewidth]{fig/oscillatory_example_2d.png}
  \caption{Oscillatory integrand of
  $\int_{-2}^3 \int_{-2}^3 \frac{e^{5 \imath \left(x^2-x y-y^2\right)}}{1 + (x+y)^2} \di{x} \di{y}  $
  with envelope $f(x) = \frac{1}{1 + (x+y)^2}$ and a quadratic oscillator
  $g(x) = x^2 - x y - y^2$ at low frequency of $\omega = 5$.}
  \label{fig:oscillatory_example_2d}
\end{figure}

For our problem at hand we will need to integrate over the whole space $\mathbb{R}^D$.
In this setting a general highly oscillatory integral takes the form:
...
\begin{equation}
  ...
  e^{- \omega \Im g(z)} \,.
\end{equation}
The main idea behind the numerical steepest descent is then to
transform the integrand such that it is no longer oscillatory but
...
and ask the question which regions of the integrand contribute most
to the value of the integral. By intuition one would say, at least asymptotically
for $\omega \rightarrow \infty$, that oscillations in integrand generally approximately
...
In the one-dimensional procedure we find all stationary points $\{x_{j}^{*}\}_{j}$
from equating the gradient to zero:
\begin{equation}
  \nabla_{x} \, g(x) = 0 \,.
\end{equation}
...
transformation $h(\tau)$. This has to be done locally at all contributing points.
Hence $\forall \xi \in \Theta$:
\begin{equation}
  g(h_\xi(\tau)) = g(\xi) + \imath \tau
\end{equation}
Solving these equations yields a bunch of paths $h_{\xi}(\tau)$. Each one is
indexed by the point $\xi$ it starts at and parametrized by $\tau \in \mathbb{R}_0^{+}$.
Refer to figure \ref{fig:nsd_path_example} for a simple example with only a single
stationary point.

\begin{figure}
  \centering
  \includegraphics[width=0.6\linewidth]{./fig/nsd_path_figure.pdf}
  \caption{Example of the contributing points $a$, $b$ and $x_0^{*}$ and the
  paths $h_a(\tau)$, $h_b(\tau)$ and $h_{x_0^{*},\pm}(\tau)$ attached.}
  \label{fig:nsd_path_example}
\end{figure}

Solving the path equation amounts to finding an inverse of $g$. In some cases
...
path to be really attached to the point $a$. At the stationary points we have
to choose two paths, $h_{x^{*}_j,+}$ and $h_{x^{*}_j,-}$, one of which is considered
incoming and the other outgoing (note that this is not reflected by the parameterization).
...
will give a bunch of integrals, one or two for each $\xi$:
...
In our case there is no danger because $f$ is simple a polynomial.
...
there can be exponentially many of these. For this type of semi-infinite integrals
Gauss-Laguerre quadrature with nodes $\{\gamma_k\}_{k}$ and weights
$\{w_k\}_{k}$ seems to be most appropriate:
\begin{equation}
  J[\xi] \approx \frac{e^{\imath \omega g(\xi)}}{\omega}
                 \sum_{k}
                   ...
                   w_k
\end{equation}
Depending on the path $h_{\xi}$ these integrals can be weakly singular which
can in turn be accounted for by the generalized Gauss-Laguerre quadrature.

For our problem we will find that we can always glue together the two opposite
paths at the single stationary point, apply one further transformation and
finally use Gauss-Hermite quadrature. The whole process for computing \eqref{eq:hoi}
then looks like shown in figure \ref{fig:transformation_chain_2}.
...
Performing these transformations on the first two example wavepackets $\phi_k$ and $\phi_l$
from figure \ref{fig:overlap_example} we can make the plot shown in \ref{fig:hawp_trafo_example}.
...
In the last part we have walked through the very basic concept of the numerical steepest
...
is worked out too by the original authors. However, the state of the theory is too narrow
for setting. First we need to consider complex stationary points too. If a such a point
is near enough the real line, it will influence the oscillator enough to become relevant.
Figure \ref{fig:oscillatory_integral_complex_sp} shows the example
\begin{equation}
  \int_{-2}^{2} \log\left(2+x^2\right) e^{50 \imath \left(\delta x + \frac{x^3}{3}\right)} \di{x}
\end{equation}
for different values of $\delta \in \{10, 5, 2, 1, \frac{1}{2}, \frac{1}{5}, \frac{1}{10}, \frac{1}{100}, 0\}$.
One stationary point is located at $x^{*} = \imath \sqrt{\delta}$ and comes close
to the real line. The influence is clearly present. The general advise for complex
stationary points is that the path has to pass through this point. It turns out this
works very well for our setting.
...
\begin{figure}
  ...
  shown for several different decreasing values of $\delta$.}
  \label{fig:oscillatory_integral_complex_sp}
\end{figure}

Another direction to extend the theory are infinite regions $\Omega$. In out case we will need
$\Omega = \mathbb{R}^D$. Before we take this step, we look at semi-finite intervals.
For semi-finite intervals $[a, \infty [$ we attach only a single path $h_a(\tau)$ an the
endpoint $a$. Details are described by Majidian and can be found in \cite{H_nsd_sii}.
As an example, we look at the following integral:
\begin{equation}
  I = \int_0^\infty \frac{100}{100 + x^2} \exp\left(\imath \omega e^{\sqrt{x}}\right) \di{x}
\end{equation}
where we set $\omega = 2$. The path then is given by the expression:
\begin{equation}
  h_a(\tau) = \log(1 + \imath\tau)^2 \,.
\end{equation}
After the change of variable the integral takes the following ugly form:
\begin{equation}
  I = 200 e^{\imath\omega}
      \int_0^{\infty}
...
We just split the interval $]-\infty, \infty[$ into the union of three
...
$a$ and $b$ will be traversed two times in opposite directions and hence cancel out.
The only thing that remains is the pair of paths attached at the complex stationary
point $x^{*}$. This works the same in any number of dimensions. By gluing together
the two paths of opposite direction we end up with a cross of exactly $D$ paths.
Therefore the number and nesting depth of integrals to perform does \emph{not} grow
compared to direct integration. This is of course very fortunate.
...
\marginpar{It is not clear if/when that can happen. Degenerate conic.}
...
First of all, despite comprehensive numerical evidence, we need a full proof
showing the validity of our extension to the theory of numerical steepest descent
to infinite intervals. The idea was to use the work from \cite{HV_hoq}
...
In this process it turned out that the proof given in the latter reference is flawed.
As is stands now we can not even assume the semi-finite part to be correct.
...
that situation in the first reference. The central observation being made is that
...
transformation. One might try to resolve these oscillations by another method on top
...
the integral for any state $\phi_k$. Evaluation is very expensive but can never the
less serve as exact reference solution. For examples in higher dimensions we can take
...
which is known combination technique. The formula was first derived in
...
with moderate effort. Even better, one can proof \cite{novak_ritter} that
...
Since there are much less Genz-Keister nodes comapred to the full tensor
of Gauss-Hermite nodes, as shown in figure \ref{fig:gk_hermitephy_ratio},
we can largely reduce the computational effort.
...
The second setup fixes momentum and varies the semi-classical scaling
...
caused by the oscillatory nature of the integrands. Depending on
the phase space constellation wavepackets, Gaussian quadrature
can introduce large errors hence entirely destroying the numerical simulation results
in the worst case.

Based on some recent developments in the area of highly oscillatory quadrature
...
$\braket{\phi|\hat{o}|\phi^\prime}$ but issues may arise from the complex behavior of $\hat{o}$.

The fundamental technique is not restricted to one-dimensional examples and works well
in any number of dimensions. There is still a quadrature involved, using a smaller number
of nodes. Full tensor product quadrature becomes expensive in higher dimensions.
Therefore we applied the Smolyak construction and use a specially crafted set of nested
