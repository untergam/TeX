...
over the whole space $\mathbb{R}^D$. In this setting a general highly oscillatory
...
finally use Gauss-Hermite quadrature. The whole process for computing \eqref{eq:oscillatory_integral}
...
stationary points is that the path should pass through this point. This will then give
an exact decomposition in the sense of \eqref{eq:cauchy}. It turns out this
works very well for our setting.

\begin{figure}
  \centering
  \includegraphics[width=0.9\linewidth]{./fig/oscillatory_integral_complex_sp.pdf}
...
endpoint $a$. Details are described by Majidian and can be found in \cite{H_nsd_sii}. If there
