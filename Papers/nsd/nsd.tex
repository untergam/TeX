...
\begin{abstract}
  In this report we show that classical Gauss quadrature is not adequate for a
...
descent method. Under some broad assumptions for $f$ and $g$ in~\eqref{eq:hoi} this was proven
to work for finite intervals and an arbitrary number of real stationary points~\cite{HV_hoq}.
The extension into higher dimensions has been worked out~\cite{HV_cub}.
...
\end{equation}
The definition~\eqref{eq:general_partial_gi} yields the left hand side:
\begin{equation}
\begin{split}
  g_i\left(h_i\right)
...
\begin{equation}
 ...
 \end{bmatrix} \,.
\end{equation}
The last relation is only a notational shortcut and
...
\begin{algorithm}
  ...
  \end{algorithmic}
\end{algorithm}
Some matrix algebra and vectorisation techniques allow us to
...
$[0, \infty[$ seems unfortunate for our goal of applying a quadrature.
...
\end{split}
\end{equation}
and glue together the two parts, so the final integral is:
\begin{equation}
...
\subsection{Oscillatory prefactors}
...
\subsection{General quadratic oscillator}
...
\subsubsection{Removing the linear term}
...
\subsubsection{Triangularize the oscillator}
...
before applying a quadrature scheme gives correct results for a much smaller number of nodes.
