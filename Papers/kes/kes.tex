...
then must happen for all summands in the sum. A simple algorithm is given in
\eqref{alg:gk_compute_zsequence} below.
...
can serve as the basis for the generator set $\Lambda$. Genz and Keister used in the Hermite
case the extensions $\mathcal{K} = (1, 2, 6, 10, 16)$ and $\mathcal{K} = (1, 2, 8, 20)$.
One should note that not all rules are well suited. A criterion on the expected \emph{sparsity}
is the number of zero elements in the $\vec{z}$ sequence. The more non-zero elements the better
but unfortunately such sequences are rare. Additionally there appear the question of stability
which was discussed in \cite{genz-keister}.


\marginpar{plot number of nodes 1D}
...
\begin{algorithm}[h!]
  ...
  \begin{algorithmic}
    \Procedure{GenzKeisterConstruction}{$D, K$}
      ...
      \State $\Omega := \{\}$
      \State $ $
      \Comment Precompute fundamental data
      \State $\Lambda := \textsc{ComputeGenerators}(\ldots)$
      \State $M := \textsc{ComputeMoments}()$
      \State $\vec{A} := \textsc{ComputeAValues}(\Lambda, M)$
...
the use of \texttt{yield} statement inside so called \emph{generator expressions}.
