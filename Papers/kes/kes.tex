...
well known truth and summarized in table \eqref{eq:orthogonal_polynomials}.
...
\begin{table}[h!]
  ...
  \begin{tabular}{|l|l|l|l|}
    ...
    Normal       & $\frac{\exp\left(-\frac{t^2}{2}\right)}{\sqrt{2\pi}}$
                 & $[-\infty, \infty]$ & Hermite \\
    \hline
  \end{tabular}
...
In the last case of the Hermite polynomial we find that for even $n$:
...
\section{Genz-Keister Constructions}


Genz found an explicit construction by which
special quadrature rules for an arbitrary number
of dimensions can be built. The resulting rules are
called \emph{fully-symmetric}.

In this section we review this construction of Genz and Keister
as given in \cite{genz, genz_keister}. We follow mostly their
notation but focus on the computational aspects.
...
The node set $\Gamma$ and weights $\omega$ are indexed by partitions
...
\begin{equation} \label{eq:partition_set_def}
  ...
  \,.
\end{equation}
An algorithm for computing the set $\mathcal{P}$
...
a subset because elements are allowed to appear multiple times. The
specific items are selected by an integer vector $\mathbf{k} \in \mathbb{N}_0^D$,
explicitly:
\begin{equation}
...
Because we require that only $\lambda_0$ is zero, this is equivalent to the number
...
Clearly, this set is of size $2^{D-\delta}$.
At this point we can go back and understand the notation of equation \eqref{eq:gk_basic_formula}
...
\end{equation}
and $\mathcal{S}_\mathbf{p}$ is the set of all permutations of the
...
What remains is the explanation of $\omega_\mathbf{p}$. The formula looks
quite easy but the efficient implementation needs some care.
For every admissible partition $\mathbf{p}$ the corresponding weight
can be computed by:
...
\end{equation}
and the multi-index $\mathbf{k} \in \mathbb{N}_{0}^{D}$. Algorithm
\eqref{alg:ca_enumerate_lattice_points} implements a procedure for enumeration
of all relevant multi-indices.
This formula presents us several parts we need to compute. Some of these parts
can even be tabulated once for a maximal value and independent of the dimension $D$.
...
appropriately (see next section). The polynomials $p_{i}(x)$ are
defined as:
\begin{align}
  p_{0}(x) & := 1 \\
...
for some coefficients $c_j$ and compute this integral term wise
by using linearity:
\begin{equation}
  a_i = \sum_{j=0}^{2i} c_j \int_\Omega x^j w(x) \di{x} = \sum_{j=0}^{2i} c_j M_j
\end{equation}
where $M_j$ is the $j$-th moment and chosen according to the table \eqref{eq:orthogonal_polynomials}.
For the Legendre and the Hermite case, the explicit formulae are
\eqref{eq:moments_legendre} and \eqref{eq:moments_hermite_pro}.
...
we can however not solve this problem exactly.
...
Computing the denominator of \eqref{eq:gk_weight_def} is straight forward.
Since we required the $\lambda_i$ to be pairwise distinct this value never
becomes zero.
...
\end{equation}
In the example below the top left corner is shown and we only need the
upper right triangular part:
...
Given the list $\Lambda$ of generators and all the values $a_{i}$ we can compute the table $\mat{T}$
efficiently by algorithm \eqref{alg:gk_compute_weight_factors}. By the use of this table, the implementation
of formula \eqref{eq:gk_weight_def} transforms into a series of trivial table look-up steps.
