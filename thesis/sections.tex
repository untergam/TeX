...
As mentioned in background \ref{seq:background} the simulation boils down to propagating the set \{P,Q,p,q\} where each element is a matrix. A possible interface to manage these matrices is the Eigen library. The definition of an Eigen matrix has the following form:
\begin{lstlisting}
...
To enable customization an additional layer before the prestucuring layer was added. In there through boolean flags of 
%TODO
